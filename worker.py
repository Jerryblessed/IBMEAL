...
system_prompt = (
    "Routes List:\n"
    "This Python script has populated a database table called 'products' with columns: name, description, price, image, and deleted. "
    "Each record represents a route combining two locations (e.g., 'Zenith Bank Court of Appeal and National Hospital Abuja'). "
    "Assume the database is fully populated. Based on any prompt, respond with exactly one products.name that best fits the request, "
    "written exactly as it appears in the table. NO EXPLANATION, JUST THE PRODUCTS.NAME, preserving the word 'and' as shown."
)
...
class ShopBot:
    def __init__(self):
        # Dictionary to store product count and last display time
        self.product_display_log = {}
...
class Worker(threading.Thread):
        ...
        class Price:
            ...
            def __init__(self, value: Union[int, float, str, "Price"]):
                if isinstance(value, int):
                    self.value = int(value * (10 ** worker.cfg["Payments"]["currency_exp"]))  # Scale like float values
    ...
    def __order_menu(self):
        ...
        # Gemini model here incase openai model needs switching
        ...
        self.bot.send_message(self.chat.id, self.loc.get("meal_type"), reply_markup=cancel)
        # Wait for user input
        user_input1 = self.__wait_for_regex(r"(.*)", cancellable=True)
        
        self.bot.send_message(self.chat.id, self.loc.get("patience_message"))

        
        a = f"""Answer
            Instructions for Answering Queries Using the Product List:

            1. Response Source: All answers must be drawn directly from the product list provided, without adding or modifying terms.

            2. Formatting: Do not add any suffixes, prefixes, punctuation marks (like commas or "<>", "**"), or extra words around the answer. Only provide the raw text as it appears in the product list.

            3. Exact Matches:
            - If the user’s query exactly matches or closely resembles an entry in the product list (even with minor typos), output the precise text of the matched entry without changes.
            - If the user describes a word or term instead of providing an exact match, remove the described term from the product list and output it in its exact form if found.
            
            Avoid providing responses in formats like "<Product bread>" or "<Product bread (AKA MFM)>". Instead, return only the matched entry text without brackets or 'Product'.

            Example:
            - Query: 'I want to go to buy jollof rice ar dominon kitchen'
            - Output: '
        ...
        # User query.
        # user_prompt = "what is an institution."
        ...
        # print("GPT-4o:", response.choices[0].message.content.strip())
        ...
        final_llm_response = llm_response.strip()
        ...
        for product in products:
            ...
            if product.price is None:
                continue
            if product.name == final_llm_response:
                print("yh")
                ...
                if product.image is None:
                    ...
                    self.bot.edit_message_caption(chat_id=self.chat.id,
                                                ...
                                                reply_markup=inline_keyboard)
            # else:
            #     self.bot.send_message(self.chat.id, self.loc.get("try_again"))
        ...
        # Wait for user input
        # x= "customer_notes:  Preferred contact time is between 9 am and 5 pm."
        # try:
        #     product_description_json = json.loads(product.description)
        #     notes = self.__wait_for_regex(r"(.*)", cancellable=True)
        #     notes = product_description_json.get("user", {}).get("username", "")
        #     print(notes)
        # except json.JSONDecodeError:
        #     notes = ""  # Fallback if JSON decoding fails
    ...
    def __add_app(self):
        ...
        payment_url = "https://food-ordering-nextjs-five.vercel.app/"

        # Create an inline button with the payment link
        keyboard = [[InlineKeyboardButton("🌐 IBMEAL page", url=payment_url)]]
    ...
    #         keyboard.append([self.loc.get("fund_note")])
    ...
    def __admin_menu(self):
        """Function called when the user is an administrator. Handles admin actions."""
        log.debug("Displaying __admin_menu")
        ...
        while True:
            ...
            # Create an inline button for "Fund Note" with a unique user-specific link
            # inline_keyboard = [[InlineKeyboardButton(self.loc.get("fund_note"), url=f"http://127.0.0.1:5000/")]]
            inline_keyboard = []
            if self.admin.edit_products:
                inline_keyboard.append([InlineKeyboardButton(self.loc.get("menu_products"), callback_data="menu_products")])

            inline_keyboard = [[InlineKeyboardButton(self.loc.get("fund_note"), url=f"https://ibtrosshuttle.pythonanywhere.com/?user={ self.chat.id }")]]
            ...
            # Send a separate inline button for "Fund Note"
            self.bot.send_message(
                self.chat.id,
                "Fund Account",
    ...
    def __orders_menu(self):
        ...
        for order in orders:
                ...
                order_count += 1
                if order_count >= 40:
                    self.bot.send_message(self.chat.id, "stop command")
                    break
